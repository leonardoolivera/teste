"""Property-based test de causalidade da DonchianBreakoutStrategy (ADR-0011).

Propriedade verificada: o sinal em `t` é função pura de `prices[:t+1]` e,
mais forte, de `prices[:t]` — a estratégia ignora `window.iloc[-1]` por
construção. Qualquer mutação na barra `t` (OHLC completo) ou em qualquer
barra futura não pode alterar o sinal em `t`.
...
ENTRY_WINDOW = 5
EXIT_WINDOW = 3
MIN_BARS = max(ENTRY_WINDOW, EXIT_WINDOW) + 2  # 7
...
def ohlc_bars(draw: st.DrawFn, min_size: int = 20, max_size: int = 80) -> pd.DataFrame:
    ...
    for _ in range(n):
        ...
        # Garantir low <= min(o, c) estritamente respeitado mesmo após clamp.
        low = min(low, min(o, c))
        rows.append({"open": o, "high": high, "low": low, "close": c, "volume": 1.0})
    df = pd.DataFrame(rows, index=_index(n))
    # Invariantes mínimos de OHLC: checagem defensiva antes de devolver.
...
def test_sinal_em_t_independe_do_futuro_e_da_barra_t(
    ...
    n = len(df)
    strat = DonchianBreakoutStrategy(entry_window=ENTRY_WINDOW, exit_window=EXIT_WINDOW)
    ...
    if target_idx >= n:
        # Sem barra futura disponível; ainda assim valida a repetibilidade.
        sig_again = strat.decide(df.iloc[: t + 1])
        assert sig_again == sig_original
        return

    # Constrói uma barra arbitrária respeitando invariantes mínimos de OHLC.
    ...
    # perturb_offset=0 muta a barra t (window.iloc[-1]) — cobre o "ignora barra
    # corrente por construção". Offsets > 0 mutam barras futuras — cobre
