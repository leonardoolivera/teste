"""Property-based test de monotonicidade de custo para `RSIMeanReversionStrategy`.

Aplicação mecânica do invariante ADR-0010 + ADR-0019 à sexta família do
laboratório (ADR-0027). Varia-se os 3 eixos de custo (fee, slippage, spread)
...
Estratégia de referência: `RSIMeanReversionStrategy(period=14, oversold=30,
overbought=70)` sobre o dataset sintético seminal.
"""
...
REFERENCE_PERIOD = 14
REFERENCE_OVERSOLD = 30.0
REFERENCE_OVERBOUGHT = 70.0
...
def _run(prices: pd.DataFrame, cost_model: CostModel) -> BacktestResult:
    return run_backtest(
        ...
        strategy=RSIMeanReversionStrategy(
            ...
            oversold=REFERENCE_OVERSOLD,
            overbought=REFERENCE_OVERBOUGHT,
        ),
        budget=RiskBudget(
...
def test_cost_monotonicity_rsi_in_final_equity(
    ...
    assert delta <= FINAL_EQUITY_TOLERANCE, (
        "Violação de monotonicidade de custo (ADR-0010 + ADR-0019 aplicados a RSI): "
        "custo maior produziu final_equity maior.\n"
        f"  strategy = RSIMeanReversionStrategy(period={REFERENCE_PERIOD}, "
        f"oversold={REFERENCE_OVERSOLD}, overbought={REFERENCE_OVERBOUGHT})\n"
