"""Property-based test de monotonicidade de custo para Donchian (ADR-0012).

Aplicação direta de ADR-0010 à família Donchian. Invariante idêntica ao
test de MA crossover: fixado o cenário, se ``cost_high`` domina
``cost_low`` componente a componente e o cenário `low` gera ao menos um
trade, então ``final_equity_high <= final_equity_low + TOLERANCE``.

Estratégia de referência: ``DonchianBreakoutStrategy(20, 10)`` sobre o
dataset sintético seminal (``synthetic_btcusdt_1h_seed42``). Mesmo
budget, mesma tolerância, mesmo padrão que
``tests/property/test_cost_monotonicity.py``.

Fecha blocker #B-2 do AUDIT.md.
"""
...
# ADR-0010 + ADR-0012: estratégia, dataset e budget são parte do cenário fixo.
...
# Tolerância numérica absoluta. Mesmo valor/razão que ADR-0010:
# ``1e-6 * capital_inicial`` absorve ruído de ponto flutuante sem mascarar
# bug de sinal.
...
def test_donchian_cost_monotonicity_in_final_equity(
    ...
    assert delta <= FINAL_EQUITY_TOLERANCE, (
        "Violacao de monotonicidade de custo (ADR-0010/0012, Donchian): "
