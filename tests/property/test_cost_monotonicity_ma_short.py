"""Property-based de monotonicidade de custo — MA crossover em modo simétrico (ADR-0010 + ADR-0012).

Aplicação mecânica do invariante da ADR-0010 ao modo `long_only=False` da
`MovingAverageCrossoverStrategy`. Arquivo paralelo, deliberadamente — segue o
mesmo padrão dos testes de MA long-only e Donchian. Quando uma terceira
variante da MA (ex.: filtro de regime, leverage dinâmica) entrar, novo arquivo
paralelo.
...
ADR-0012 §"custo duplo na reversão" torna esta invariante **ainda mais
sensível ao custo**: cada reverse-on-signal paga taker + slippage duas vezes
(fechamento + abertura), portanto `cost_high` drena `final_equity` mais
agressivamente que no modo long-only. Se a invariante valer aqui, vale
também pela arquitetura do engine, não por coincidência numérica.

Estratégia de referência: `MovingAverageCrossoverStrategy(20, 50, long_only=False)`
...
def _run(prices: pd.DataFrame, cost_model: CostModel) -> BacktestResult:
    return run_backtest(
        ...
        strategy=MovingAverageCrossoverStrategy(
            short_window=REFERENCE_SHORT_WINDOW,
            long_window=REFERENCE_LONG_WINDOW,
            long_only=False,
        ),
        budget=RiskBudget(
...
def dominated_cost_pair(draw: st.DrawFn) -> tuple[CostModel, CostModel]:
    """Mesma composite usada nos testes MA long-only e Donchian."""
    fee_low = draw(
        st.floats(
            min_value=0.0,
...
def test_cost_monotonicity_ma_short_in_final_equity(
    ...
    assert delta <= FINAL_EQUITY_TOLERANCE, (
        "Violação de monotonicidade de custo (ADR-0010 aplicado a MA long_only=False, ADR-0012): "
        "custo maior produziu final_equity maior.\n"
        f"  strategy = MovingAverageCrossoverStrategy(short={REFERENCE_SHORT_WINDOW}, "
        f"long={REFERENCE_LONG_WINDOW}, long_only=False)\n"
