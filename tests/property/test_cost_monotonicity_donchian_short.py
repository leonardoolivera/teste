"""Property-based de monotonicidade de custo — Donchian em modo simétrico (ADR-0010 + ADR-0013).

Aplicação mecânica do invariante da ADR-0010 ao modo `long_only=False` da
`DonchianBreakoutStrategy`. Arquivo paralelo, deliberadamente — segue o mesmo
padrão dos outros três arquivos de monotonicidade (MA long-only, Donchian
long-only, MA simétrico). Quando uma quarta variante da Donchian (ex.: filtro
de regime, janelas assimétricas por direção) entrar, novo arquivo paralelo.
...
ADR-0013 reusa reverse-on-signal da ADR-0012 (§"custo duplo na reversão") sem
alterar o engine: cada reversão no modo simétrico da Donchian paga taker +
slippage duas vezes (fechamento + abertura no mesmo `ts_exec`). Portanto
`cost_high` drena `final_equity` mais agressivamente que no modo long-only.
Se o invariante vale aqui, vale por arquitetura do engine, não por
coincidência numérica.

Estratégia de referência: `DonchianBreakoutStrategy(20, 10, long_only=False)`
...
def dominated_cost_pair(draw: st.DrawFn) -> tuple[CostModel, CostModel]:
    """Mesma composite usada nos três testes paralelos."""
    fee_low = draw(
        st.floats(
            min_value=0.0,
            max_value=FEE_BPS_MAX,
            allow_nan=False,
    ...
    slip_delta = draw(
        st.floats(
            min_value=0.0,
            max_value=SLIP_BPS_MAX - slip_low,
            allow_nan=False,
            allow_infinity=False,
        )
    )
...
def test_cost_monotonicity_donchian_short_in_final_equity(
    ...
    assert delta <= FINAL_EQUITY_TOLERANCE, (
        "Violação de monotonicidade de custo (ADR-0010 aplicado a Donchian long_only=False, ADR-0013): "
        "custo maior produziu final_equity maior.\n"
        f"  strategy = DonchianBreakoutStrategy(entry={REFERENCE_ENTRY_WINDOW}, "
        f"exit={REFERENCE_EXIT_WINDOW}, long_only=False)\n"
