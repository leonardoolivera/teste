"""Serie CG snowball — re-run CG com --sizing-mode snowball (ADR-0063).

9 pilotos identicos a CG (Bollinger short + width 300), unica mudanca: sizing snowball.
Comparacao combo-a-combo vs CG fixed ja arquivado.
"""
...
if __name__ == "__main__":
    for tag, ds, asset, suffix in PILOTS:
        rid = f"cg-snow-bol-20-15-{asset}-{suffix}-width30-300-short"
        print(f"\n{'=' * 70}\n{tag} snowball -- {asset} {suffix}\n{'=' * 70}")
        run_one(rid, ds)
    print("\nSerie CG snowball completa (9 runs).")
