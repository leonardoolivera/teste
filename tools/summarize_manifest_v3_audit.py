"""Summarize manifest v3 audit — aplica Gates A/B/C de ADR-0059."""
...
APPROVED = [
    ("CG.3", "sol", "20240705_20241231"),
    ("CG.4", "btc", "20250105_20250704"),
    ("CG.5", "eth", "20250105_20250704"),
    ("CG.6", "sol", "20250105_20250704"),
]

EXCLUDED_SEVERE = [
    ("CG.1", "btc", "20240705_20241231"),
    ("CG.9", "sol", "20250705_20251231"),
]
...
if __name__ == "__main__":
    print("\n=== Manifest v3 audit — Gates A/B/C (ADR-0059) ===\n")
    ...
    for tag, asset, suffix in APPROVED:
        ...
        # seed 42 ja existe em cg-bol-*
        for seed, rid in [
            (42, f"cg-bol-20-15-{asset}-{suffix}-width30-300-short"),
            (1337, f"audit-v3-a-{asset}-{suffix}-seed1337"),
            (2024, f"audit-v3-a-{asset}-{suffix}-seed2024"),
        ]:
            m = load_run(rid)
            if m is None:
    ...
    a_ok = a_pass == a_total
    print(f"\nGate A: {a_pass}/{a_total} {'PASS' if a_ok else 'FAIL'}")
    ...
    for tag, asset, suffix in APPROVED:
        rid_with = f"cg-bol-20-15-{asset}-{suffix}-width30-300-short"
        rid_without = f"audit-v3-b-{asset}-{suffix}-nofilter"
    ...
    audit_c_results = []
    c_still_fail = 0
    for tag, asset, suffix in EXCLUDED_SEVERE:
    ...
    c_ok = c_still_fail == len(EXCLUDED_SEVERE)
    print(f"\nGate C: {c_still_fail}/{len(EXCLUDED_SEVERE)} ainda FAIL com seed 1337 {'PASS' if c_ok else 'FAIL'}")

    overall = "PASS" if (a_ok and b_ok and c_ok) else "FAIL"
    print(f"\n{'=' * 50}\nMANIFEST V3 AUDIT: {overall}\n{'=' * 50}")
    if overall == "PASS":
        print("  => Marcar live_status=active, notificar BotBinance.")
    else:
        print("  => Reverter promocao: live_status=blocked.")
        if not a_ok:
            print(f"     Audit A falhou: {a_pass}/{a_total}")
        if not b_ok:
    ...
    out_p = Path("exports/diag/manifest_v3_audit_summary.json")
    out_p.parent.mkdir(parents=True, exist_ok=True)
    out_p.write_text(json.dumps({
        "audit_adr": "0059",
        "manifest": "bollinger_short_width_20260419.json",
        "gate_a_pass": a_pass,
