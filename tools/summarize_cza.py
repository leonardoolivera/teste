"""Resumir Serie CZA (RSI short + width rescue DOT/AVAX 2025-H2)."""
...
OUT = ROOT / "exports" / "diag" / "series_cza_summary.json"

PILOTS = [
    ("CZA.1", "cza-dot-rsi-short-width300-2025h2", "DOT", "2025-H2", 0.498),
    ("CZA.2", "cza-avax-rsi-short-width300-2025h2", "AVAX", "2025-H2", -0.054),
]
...
def main():
    rows = []
    for tag, rid, asset, window, naked_sh in PILOTS:
        m = metrics(rid)
        delta = m["sharpe"] - naked_sh
        row = {"tag": tag, "asset": asset, "window": window, "run_id": rid,
               "naked_sh": naked_sh, "delta_sh": delta, **m, "verdict": verdict(m)}
        rows.append(row)
    OUT.parent.mkdir(parents=True, exist_ok=True)
    OUT.write_text(json.dumps(rows, indent=2))
    print(f"{'Tag':<7} {'Asset':<5} {'Tr':>4} {'PnL%':>7} {'MDD%':>6} {'Sh':>7} {'Nkd':>6} {'Delta':>6} {'MCp5':>6} {'costr':>6}  Verdict")
    for r in rows:
        ...
        print(f"{r['tag']:<7} {r['asset']:<5} {r['trades']:>4} {r['pnl_pct']:>7.2f} "
              f"{r['mdd_pct']:>6.2f} {r['sharpe']:>7.3f} {r['naked_sh']:>6.3f} {r['delta_sh']:>+6.3f} "
              f"{mc:>6.0f} {cr:>6.4f}  {r['verdict']}")
    passes = sum(1 for r in rows if r["verdict"] == "PASS")
    print(f"\nPASS count: {passes}/2")
    print(f"\nGate 2 load-bearing (Padrao 12): delta_sh >= +0.5 requerido para promocao.")
    for r in rows:
        tag = "LOAD-BEARING" if r['delta_sh'] >= 0.5 else "NOT load-bearing"
        print(f"  {r['asset']}: delta={r['delta_sh']:+.3f} -> {tag}")
