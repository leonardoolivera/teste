"""Serie CH — RSI short + width (min_width_bps=300) cross-period (9 runs).

Gate pre-registrado em ADR-0061. Testa genericidade do padrao 10 (ADR-0057).
"""
...
if __name__ == "__main__":
    for tag, ds, asset, suffix in PILOTS:
        rid = f"ch-rsi-14-30-70-{asset}-{suffix}-width30-300-short"
        print(f"\n{'=' * 70}\n{tag} RSI short + width300 -- {asset} {suffix}\n{'=' * 70}")
        run_one(rid, ds)
    print("\nSerie CH completa (9 runs).")
