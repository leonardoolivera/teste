"""Resumir Serie CZ (RSI short naked altcoins 2025-H2)."""
...
OUT = ROOT / "exports" / "diag" / "series_cz_summary.json"

PILOTS = [
    ("CZ.1", "cz-dot-rsi-short-naked-2025h2", "DOT", "2025-H2"),
    ("CZ.2", "cz-avax-rsi-short-naked-2025h2", "AVAX", "2025-H2"),
    ("CZ.3", "cz-link-rsi-short-naked-2025h2", "LINK", "2025-H2"),
]

# Comparacao v4b majors 2025-H2
V4B_REF = {"BTC": 1.640, "SOL": 2.300}
...
def main():
    ...
    print(f"\nPASS count: {passes}/3")
    print(f"\nGate 2 vs v4b majors 2025-H2:")
    print(f"  BTC Sh={V4B_REF['BTC']:.3f}  SOL Sh={V4B_REF['SOL']:.3f}")
    for r in rows:
        if r['verdict'] == 'PASS':
            avg = (V4B_REF['BTC'] + V4B_REF['SOL']) / 2
            delta = r['sharpe'] - avg
            print(f"  {r['asset']} Sh={r['sharpe']:.3f}  (delta vs v4b_avg={delta:+.3f})")
