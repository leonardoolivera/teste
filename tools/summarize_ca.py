"""Extrai metrics da Serie CA e avalia gates pre-registrados ADR-0039."""
...
def evaluate_gates_ca(m: dict) -> tuple[bool, list[str]]:
    """Gate ADR-0039: trades>=30, Sharpe>=0.8, MDD<=25%, fe>9500, cost_r>=0.92, MC_p5>8500."""
    fail = []
    if m["trades"] < 30:
        fail.append("trades")
    if m["sharpe"] < 0.8:
...
if __name__ == "__main__":
    ...
    PILOTS = []
    idx = 1
    for atr_bps in [100, 80]:
        for suffix in ["20230705_20231231", "20240105_20240704", "20240705_20241231",
                       "20250105_20250704", "20250705_20251231"]:
            tag = f"CA.{idx}"
            run_id = f"ca-donchian-20-10-sol-{suffix}-atrbps{atr_bps}"
            PILOTS.append((tag, run_id, suffix, atr_bps))
            idx += 1

    print("\n=== Serie CA — Donchian SOL cross-period (ADR-0039 gate) ===")
    print(f'{"Tag":6}{"Period":20}{"atrbps":>8}{"trd":>5}{"Sh":>7}{"MDD%":>7}{"fe":>9}{"cost_r":>8}{"MCp5":>9}  verdict')
    print("-" * 108)
    results = []
    passes = 0
    for tag, run_id, suffix, atr_bps in PILOTS:
        out = base / run_id
        ...
        ok, fail = evaluate_gates_ca(m)
        verdict = "PASS" if ok else "FAIL(" + ",".join(fail) + ")"
        if ok:
            passes += 1
        print(f'{tag:6}{suffix:20}{atr_bps:>8}{m["trades"]:>5}{m["sharpe"]:>7.2f}{m["mdd_pct"]:>7.2f}{m["final_eq"]:>9.0f}{m["cost_r"]:>8.4f}{m["mc_p5"]:>9.0f}  {verdict}')
        results.append(dict(tag=tag, period=suffix, atr_bps=atr_bps,
                            **{k: round(v, 4) if isinstance(v, float) else v for k, v in m.items()},
                            verdict="PASS" if ok else "FAIL", fail_reasons=fail))
    print("-" * 108)
    print(f"\nPASS count: {passes}/10 (gate ADR-0039: >=6 required)")
    print(f"SERIE VERDICT: {'PASS' if passes >= 6 else 'FAIL'}")
    out_p = Path("exports/diag/ca_series_summary.json")
    out_p.parent.mkdir(parents=True, exist_ok=True)
    out_p.write_text(json.dumps({"gate_adr": "0039", "passes": passes, "total": 10,
