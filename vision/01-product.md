...
- **Cobertura e repetibilidade do pipeline:** rodar backtest + walk-forward + Monte Carlo + stress de custos de ponta a ponta em **< 10 min** para 1 estratégia, 1 ativo, 1 timeframe e 2 anos de histórico, em ambiente local de desenvolvimento padrão. _(Calibração 2026-04-17: backtest isolado sobre 4320 barras 1h (180 dias) leva ≈ 0.9 s no host de referência; extrapolando linearmente para 2 anos e assumindo walk-forward com ~30 folds + Monte Carlo de 1000 resamples + stress de custos em 5 pontos, o pipeline hoje caberia folgadamente em < 10 min. Meta permanece plausível com o engine Python atual, mesmo antes de vectorbt (ADR-0001). Calibração fina definitiva só após ADR-0003 entregar `validation/` e o script do pipeline completo existir.)_
...
- **Capacidade real de produzir candidatas sérias:** ao menos **3 famílias de estratégias** rodando ponta a ponta até score final, e ao menos **1 estratégia candidata** que sobreviva a walk-forward + Monte Carlo + stress de custos em **≥ 2 regimes distintos**. _(Critério de sobrevivência operacional calibrado em 2026-04-18 via ADR-0025: `canary_only` exige `hit_rate ≥ 45%` absoluto; `paper_only` exige top-3 por `composite_score` de ADR-0024 em sample N ≥ 9. Série H fechou com 12 pilotos trend-following sobre BTC/ETH/SOL em plateau 24-32% hit — nenhum em `canary_only`; 3 em `paper_only` via ranking relativo. A meta "≥ 2 regimes distintos" permanece — a barra de "sobreviver" é agora híbrida: absoluta para exposição de capital, relativa para priorização.)_
