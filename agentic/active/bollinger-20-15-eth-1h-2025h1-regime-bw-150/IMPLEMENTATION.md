# IMPLEMENTATION.md - AK.3

## Mapeamento SPEC -> execucao

Implementacao nova: `BollingerWidthFilter` em `src/alpha_forge/regimes/filter.py` (extensao aditiva ADR-0022, pre-autorizada em §Consequences). Parser de `--regime-filter` estendido. `alpha-forge validate --strategy bollinger --bollinger-num-std 1.5 --regime-filter bollinger_width:window=20:num_std=1.5:min_width_bps=150 ...`.
