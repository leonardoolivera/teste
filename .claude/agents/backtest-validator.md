...
1. `AGENTS.md` + `CLAUDE.md` + `agentic/README.md`.
2. `agentic/active/<slug>/SPEC.md` + `agentic/active/<slug>/IMPLEMENTATION.md` do piloto.
3. `decisions/0002-anti-lookahead-as-infrastructure.md`, `0005-dataset-versioning-and-manifest.md`, `0006-minimal-execution-cost-model.md`, `0007-minimal-backtest-metrics.md`, `0010-cost-monotonicity-property-test.md`, `0014-systematic-cost-stress.md`, `0015-validation-report-persistence.md`, `0016-validation-cli-subcommand.md`, `0017-run-metadata-persistence.md`, `0018-validation-compare-subcommand.md`, `0019-synthetic-spread-cost-component.md`.
...
4. **Sensibilidade de custos (três eixos, ADR-0019)**: rodar com `taker_fee_bps` variando {0, 5, 10, 20}, `slippage_bps_per_notional` variando {0, 2, 5, 10}, e `spread_bps` variando {0, 5, 10}. Via `alpha-forge validate` com `--stress label:fee:slip[:spread]` ou grid externo. Observar o delta e confirmar monotonicidade (ADR-0010 estendida).
5. **Pipeline completo (ADR-0016)**: rodar `alpha-forge validate --run-id <slug> --stress ...` pra produzir walk-forward + Monte Carlo + cost_stress persistidos; usar `--mc-seed` fixo pra reprodutibilidade.
6. **Checagem de lookahead**: confirmar que o property-based de causalidade está verde e cobre OHLCV completo (não só um campo).
7. **Robustez inicial**: quando houver múltiplos datasets/janelas, rodar em pelo menos dois e reportar estabilidade. Usar `alpha-forge compare run_a run_b` (ADR-0018) pra diff entre corridas.
...
1. `agentic/active/<slug>/VALIDATION.md` e `agentic/active/<slug>/BACKTEST.md` preenchidos.
2. Corridas persistidas em `results/validation/<slug>/` (ADR-0015/0017).
3. Resumo de até 200 palavras ao orquestrador: suite N passed, métricas-chave, sensibilidade mais notável, qualquer alerta.
